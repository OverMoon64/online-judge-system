...
Streamlit 前端在屏幕顶部提供账户概览、题库与评测、AI 智能命题和后台管理导航，并显示当前用户名、中文角色与退出按钮；入口按角色精简，权限仍由后端最终裁决。题库与在线评测合并在同一工作区，代码编辑器提供行号、Tab 缩进、括号补全、语法高亮、查找替换和 VS Code 快捷键。每条提交记录可直接查看源代码与逐测试点结果，使用统一颜色区分 AC、WA、TLE、MLE、RE、CE、UNK 与 pending。
...
最终在 Ubuntu 22.04、Python 3.10.12、G++ 11.4.0 环境运行 28 个测试，结果为 28 passed；后端总行覆盖率为 91.41%，超过 CI 设置的 85% 门槛。Ruff 静态检查和格式检查均通过。
