...
模型输出必须通过严格的 Pydantic 题目结构校验；参考解法还需在同一受限判题器中跑过全部生成测试点。服务请求临时失败时自动重试两次；结构校验失败时允许一次带错误反馈的修复请求，仍失败则任务进入 failed。取消接口直接取消 asyncio 任务，传播到正在进行的 httpx 请求和本地试运行。
...
最终在 Ubuntu 22.04、Python 3.10.12、G++ 11.4.0 环境运行 26 个测试，结果为 26 passed；后端总行覆盖率为 91.61%，超过 CI 设置的 85% 门槛。Ruff 静态检查和格式检查均通过。
