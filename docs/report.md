...
Streamlit 前端按账户中心、题目中心、提交评测、AI 命题和后台管理分组。登录后侧边栏显示当前用户名与中文角色，并根据角色精简入口；权限仍由后端最终裁决。题目列表展示难度、标签、提交数量及已通过/已尝试状态，详情页紧凑呈现时间、内存、样例和代码提交入口。提交详情使用统一颜色区分 AC、WA、TLE、MLE、RE、CE、UNK 与 pending，不阻塞其它页面。
...
![Streamlit 在线评测系统管理员后台](assets/ui-overview.png)

图 1　登录后保持 Session 的管理员后台管理界面
...
最终在 Ubuntu 22.04、Python 3.10.12、G++ 11.4.0 环境运行 24 个测试，结果为 24 passed；后端总行覆盖率为 92.70%，超过 CI 设置的 85% 门槛。Ruff 静态检查和格式检查均通过。
