...
模型设置包括配置名、兼容接口 URL、模型名、API Key、输入/输出单价、计价单位和币种。每位用户可以保存并选择多个模型；完整配置使用由 `OJ_SESSION_SECRET` 派生的密钥加密写入本机文件，服务重启后无需重新输入。API、数据库和日志只返回是否已配置，不返回密钥本身；系统重置会删除加密配置。验收可使用任意 OpenAI-compatible 服务，程序未硬编码厂商。

AI 任务包含四个可观察阶段：需求分析、题面与参考解法、边界测试点、结构校验与试运行。隐藏测试点默认由模型结合算法复杂度、边界规模与预计运行时长自动决定 2–10 个，用户也可手动指定数量；样例与隐藏测试点分别存储。模型请求优先采用 OpenAI-compatible SSE 增量响应，前端实时显示当前阶段输出；若兼容服务不支持流式参数，则自动回退普通 Chat Completions 响应。每次调用聚合 prompt/completion/total Token；若提供商不返回 usage，界面明确标记无法精确统计。费用依据配置的计价单位计算。

模型输出必须通过严格的 Pydantic 题目结构校验，并同时提供 Python 3 与 C++14 参考解法；两份代码都需在同一受限判题器中跑过全部生成测试点。服务请求临时失败时自动重试两次；结构校验失败时允许一次带错误反馈的修复请求，仍失败则任务进入 failed。取消接口直接取消 asyncio 任务，传播到正在进行的 httpx 流式请求和本地试运行。

## 七、前端与交互

Streamlit 前端在屏幕顶部提供账户概览、题库与评测、AI 智能命题和后台管理导航，并显示当前用户名、中文角色与退出按钮；入口按角色精简，权限仍由后端最终裁决。题库与在线评测合并在同一工作区，默认“题目与提交”页面连续展示完整题面、限制、样例、代码编辑器和提交记录。编辑器提供行号、Tab 缩进、括号补全、语法高亮、查找替换和 VS Code 快捷键。每条记录保留 AC、WA、TLE、MLE、RE、CE、UNK 或 Pending 的缩写与中文解释，并使用独立颜色；详情可查看源代码和逐测试点结果。
...
AI 模块默认打开“智能命题”，允许从多个已保存配置中选择模型；“模型配置”页展示各模型累计输入/输出 Token 与人民币费用。任务完成后先展示题目、参考解法和测试点预览，再由用户决定是否载入题目新增表单并人工审阅。

![Streamlit 在线评测系统题库与评测](assets/ui-overview.png)

图 1　顶部导航及题面、编辑器、彩色提交记录一体化工作区
...
最终在 Ubuntu 22.04、Python 3.10.12、G++ 11.4.0 环境运行 38 个测试，结果为 38 passed；后端总行覆盖率为 91.09%，超过 CI 设置的 85% 门槛。Ruff 静态检查和格式检查均通过。
