...
进入 AI 模块后默认打开“智能命题”，可从已配置模型中选择一个执行任务。命题任务依次执行
需求分析、题目/参考解法生成、Pydantic 结构校验、测试点去重、受限运行参考解法和一次自动
修正；模型服务临时失败时自动重试两次。页面每秒刷新进度；“中断任务”会取消后台协程和
正在进行的 HTTP 请求，不是只停止动画。页面分别显示输入/输出 Token 与按配置价格计算的
人民币费用；模型未返回 usage 时会明确标记无法精确统计。

题库与评测整合在同一顶部入口。在线编辑器提供行号、Tab 缩进、括号补全、语法高亮、查找
替换和 VS Code 快捷键；每条提交记录可直接查看源代码和逐测试点结果。
