...
只停止动画。页面分别显示输入/输出 Token 与按配置价格计算的人民币费用；模型未返回 usage
时会明确标记无法精确统计。

题库与评测整合在同一顶部入口；默认“题目与提交”工作台会连续展示完整题面、限制、样例、
在线编辑器和个人提交记录，无需在题目详情与作答页之间跳转。编辑器提供行号、Tab 缩进、括号
补全、语法高亮、查找替换和 VS Code 快捷键；记录以独立颜色区分 AC、WA、TLE、MLE、RE、
CE、UNK 与 Pending，点击“详情”可查看源代码和逐测试点结果。
