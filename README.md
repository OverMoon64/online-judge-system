...
需求分析、题目/参考解法生成、Pydantic 结构校验、完整输入检查、压力数据本地展开、测试点去重、
双语言交叉试运行和最多两次自动
修正；每道题同时生成 Python 3 与 C++14 参考程序。C++ 只编译一次，两份程序会运行全部样例和
隐藏测试点；当它们逐点输出一致且至少有一个模型原答案可作为正确安全锚点时，系统会用共识结果
校准错误或空白答案，并在预览中列出修改前后明细。双解不一致、均失败或无安全锚点时禁止自动
回填并把实际状态、输入和输出反馈给模型修复。该校准完全在本地完成，不增加 Token 或费用。模型请求优先
...
需要考验复杂度时，模型只生成短小、确定性的 Python 压力输入生成器，由受限判题进程在本地展开
1–3 个大规模隐藏点，避免把数万项数据塞进模型响应。Python/C++ 正解先共同计算标准答案，系统再按
正解的实测时间与内存收紧自动资源限制，并运行一份答案正确但复杂度较低的探针解；只有探针通过小样例、
且在所有压力点均因 TLE/MLE 被淘汰，复杂度验证才算通过。压力不足时只重生成压力生成器、探针和复杂度
契约，不重写整题。新 AI 题目由服务端依次编号为 `AI0001`、`AI0002`……，修改已有题目时保持原 ID。
...
审计、重置、AI 用量、实际取消、双语言答案校准和前端 Cookie 跨 rerun/退出回归。GitHub Actions 会在
